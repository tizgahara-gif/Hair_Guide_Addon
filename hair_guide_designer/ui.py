...
def _display_mode_buttons(layout, scene):
    ...
    for mode in ('CURVE','SOLID','CARD','FLAT_MESH'):
...
class HGD_PT_card_flat_preview(HGD_PT_base):
    ...
    def draw(self, context):
        ...
        box.operator('hgd.load_card_preview_settings_to_scene', text='現在のPreview設定を読み込み', icon='IMPORT')
        box.operator('hgd.apply_card_width_preset')
        for p in ['hair_card_width_root_cm','hair_card_width_mid_cm','hair_card_width_tip_cm','hair_card_mid_position','hair_card_width_interpolation']: box.prop(scene,p)
        ...
        box.operator('hgd.update_card_previews_from_curves', text='CARD Previewを現在設定で更新')
...
class HGD_PT_output_mesh(HGD_PT_base):
    ...
    def draw(self, context):
        ...
        preview = flat.box()
        preview.label(text='Flat Preview', icon='MESH_DATA')
        preview.operator('hgd.update_flat_mesh_previews_from_curves', text='Flat Mesh Preview更新')
        preview.operator('hgd.clear_flat_mesh_previews', text='Flat Mesh Preview削除')
        flat.label(text='扁平メッシュの幅と分割数はCARD設定を使用します。', icon='INFO')
        flat.label(text='Root/Mid/Tipやサンプル数はCARD設定を使用します。')
