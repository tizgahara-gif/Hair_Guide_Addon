...
5. **カーブ編集** `[CURVE]` / `CURVE_BEZCURVE`: 毛束長さ、太さ、解像度、テーパー、個体差、形状適用、選択カーブ設定読み込み、配置点から根元更新、ツイスト更新、低頻度のハンドルねじり。
6. **CARD表示** `[CARD]` / `MESH_PLANE`: 表示モード、CARD幅プリセット、Root/Mid/Tip幅、同期幅、CARDプレビュー更新、編集Curveを開く、選択対象への表示モード適用。CARDサンプル数など低頻度設定は **詳細を表示** に格納します。
...
表示方式は **CARD表示**、太さ・Taper・位置ブレ・長さブレなどの数値は **カーブ編集** に集約されています。
...
**カーブ編集 > ハンドルねじり** は、Root/Mid/Tipなど各Bezier制御点の座標を維持したまま、Bezierハンドルだけを接線軸まわりに回転してCurve形状そのものへ軽いツイスト感を与える低頻度機能です。Object Origin、Object Location、各制御点座標は変更しません。対象は通常Curveとツイスト制御Curveです。表示用 `twist_strand`、CARD Preview、CARD Mesh、Flat Meshを直接変形する機能ではありません。

設定項目:

- ハンドルねじり角: 根元から毛先へ分布させるハンドル回転角です。
- ハンドルねじり強度: 既存ハンドル長を基準に、ねじり後のハンドル長へ掛ける倍率です。
- ねじり分布: 線形、中間最大、毛先最大から選びます。
- 両端ハンドルを弱める: 根元と毛先の破綻を避けるため、両端のねじりを弱めます。

CARD Preview、CARD Mesh、Flat Meshを選択して **選択Curveのハンドルをねじる** を押した場合は、保存された参照元をたどって編集Curveへ適用します。通常CARDは元の通常Curveへ、ツイストCARDは表示用 `twist_strand` ではなく `twist_control` へ適用します。CARD Previewはリアルタイム追従しないため、適用後に **CARDプレビューを更新**、または **選択対象へ表示モード適用** を押して見た目を再生成してください。
...
CARD Previewが途中で反転する場合は、CARD表示 > ねじれ修正 から「CARDねじれ修正」を実行します。

この機能はParallel Transport Frameにより、Curveに沿ってCARD幅方向を連続的に計算します。

カード面全体の向きが合わない場合は、CARD標準ロール角を調整し、「CARD Rollを適用」を押してください。

CARD Preview / CARD Mesh / Flat Meshを選択している場合も、参照元Curveを解決して処理されます。
