...
カーブ形状設定内の三つ編み設定・ツイスト設定は折りたたみ式です。通常は非表示にしておき、三つ編みやツイストを調整する時だけ展開してください。将来互換用の根元半径・毛先半径・先細り強度も **詳細/互換設定** に格納されています。
...
丸断面は `bevel_depth` を使います。扁平断面は `HGD_Flat_Profile` Profile Objectを使い、`HGD_Default_Taper` と併用してテーパーを維持したまま横長の板状・楕円状に見せる簡易表示です。

FLAT断面はProfile Objectに依存します。Profile Objectが作成できない、または不整合がある場合はROUNDへFallbackし、表示太さを戻してCurveが見えなくなることを防ぎます。

環境やCurve状態によって扁平断面が表示されない場合があるため、暫定的に丸断面へFallbackする処理を入れています。

安定した扁平出力が必要な場合は、**選択カーブを扁平メッシュ化** または **全カーブを扁平メッシュ化** を使用できます。この機能は表示用Curve（通常Curve / 三つ編み表示Curve / ツイスト表示Curve）を評価サンプリングし、楕円断面リングを連続配置した `HGD_FLAT_MESH_元Curve名` Meshを `HairGuideSystem/FlatMeshes` に生成します。元Curveは削除されず、三つ編み制御Curveとツイスト制御Curveは対象外です。
...
現在の方式は、単純な3本ツイストではなく、編み目ユニットを連続配置する方式です。3本の房が `LEFT / CENTER / RIGHT` のレーンを移動し、中央通過時に前後差を付けることで三つ編みらしい重なりを表現します。輪っか状・鎖状に見えないよう、中央通過時は横幅を少し圧縮します。

三つ編み制御カーブは形状制御専用です。髪として表示されるのは自動生成される3本の三つ編み表示カーブです。制御カーブはWire表示・最前面表示で、Taperや太さ一括変更の対象にはしません。

三つ編み表示用カーブ `braid_strand` は、制御カーブから再生成されます。`braid_strand` を個別編集しても、三つ編み更新時に削除・再生成されます。形状調整は `HGD_BRAID_CTRL` を編集してください。
...
- 編み目数: 交差ユニット数。1つが1回の編み目です。
- 横幅: 三つ編み全体の左右幅。
- 交差の奥行き: 中央交差時の前後方向のふくらみ。大きいほど房の上下差が強く見えます。
- 毛先細り: 毛先へ向かう細り具合。
- 編み込み倍率: 編み目パターンの進み方。通常は1.0で使用します。
- 解像度: 制御カーブのサンプリング密度。
...
`braid_strand` / `twist_strand` は制御カーブから再生成されるため、表示用カーブを直接編集しても更新時に失われます。形状調整は `HGD_BRAID_CTRL` または `HGD_TWIST_CTRL` を編集してください。
...
- 配置点から更新: 配置点を移動した後、通常カーブ、三つ編み制御カーブ、ツイスト制御カーブの根元を追従させます。
- 選択三つ編みを更新 / 全三つ編みを更新: 三つ編み制御カーブから表示用Curve 3本を再生成します。
- 選択ツイストを更新 / 全ツイストを更新: ツイスト制御カーブから表示用Curveを再生成します。

形状適用の対象は通常カーブ、三つ編み表示カーブ、ツイスト表示カーブです。三つ編み制御カーブとツイスト制御カーブは形状制御専用のため、断面・Taper・太さ適用の対象外です。
...
Top / Front / Side_L / Side_R / Back_Upper / Back_Middle / Nape / Braid / Twist に分かれるため、Outliner上で管理しやすくなります。
...
ガイドや配置点、表示用カーブが頭部メッシュに隠れる場合は、**表示と削除** パネルの **最前面表示にする** を押してください。解除したい場合は同じ場所に表示される **最前面表示を解除** を押します。この設定は新規生成されるガイド、領域線、配置点、警告、通常Curve、三つ編み表示Curve、ツイスト表示Curveにも反映されます。

最前面表示トグルは、ガイド・配置点・警告・通常Curve・三つ編み表示Curve・ツイスト表示Curveに適用されます。三つ編み制御Curveとツイスト制御Curveは編集用のため、常に最前面表示されます。
