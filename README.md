...
毛先長さは新規Curve生成時のみ使用する初期値です。生成済みCurveへ再適用しても変更されません。生成後の長さ調整は通常のCurve編集で行ってください。

■ 表示モード

表示モードはクイック操作Panelへ集約されています。通常作業の流れで、Curve / Solid / CARDを選び、選択対象または全Curveへ適用します。

■ 頭頂配置点

Top / 頭頂部（およびBack_Upper / 後頭部上層）の配置点は、頭部最高点より少し上に生成されます。

■ ツイスト設定

ツイスト設定の距離系UIはcm単位表示です。内部保存・計算は従来通りm単位を維持し、既存Blend互換を保ちます。
