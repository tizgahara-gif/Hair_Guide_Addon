...
6. **CARD表示** `[CARD]` / `MESH_PLANE`: 表示モード、CARD幅プリセット、Root/Mid/Tip幅、同期幅、CARDプレビュー更新、CARD Control Emptyによる方向制御、編集Curveを開く、選択対象への表示モード適用。CARDサンプル数など低頻度設定は **詳細を表示** に格納します。
...
EmptyのローカルX軸がCARDの幅方向として使われます。Empty未設定の場合は、自動フレーム方式（Parallel Transport Frame + CurveごとのRoll値）で生成されます。CARD Preview / CARD Mesh / Flat Meshを選択して作成・割り当てを行った場合も、保存された `hair_source_curve` から参照元Curveを解決し、そのCurveへ `hair_card_control_empty` が保存されます。
