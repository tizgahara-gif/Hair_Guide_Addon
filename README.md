...
根元の位置ブレは小さく、毛先の位置ブレは大きく設定できます。長さのブレは根元を基準に適用されるため、Placement Point自体は動きません。長さのブレ `0.15` は、約85%〜115%の長さ差を意味します。既存Curveへブレを繰り返し加えると形状が累積して崩れるため、形状適用では位置ブレの再ランダム化を行いません。
