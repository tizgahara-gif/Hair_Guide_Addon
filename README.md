...
表示方式は **表示モード**、太さ・Taper・位置ブレ・長さブレなどの数値は **カーブ形状設定** に集約されています。
...
- CARDプレビュー: Curveを維持したまま板ポリ状に確認する制作中表示です。

CARDプレビューは `HairGuideSystem/CardPreviews` に追従用一時Meshを作る非破壊確認用プレビューです。CARDプレビューをON/OFFしても、出力済みの扁平メッシュや実体化済みCARD Meshは変更されません。CARD幅はcm単位で入力し、内部計算ではBlender標準のmへ変換します。
...
CARD実体化では、CARDプレビューと同じ板ポリ形状を `HGD_CARD_MESH_元Curve名` Meshとして `HairGuideSystem/CardMeshes` に生成します。元CurveとCARDプレビューは削除されません。既存同名Objectがある場合はBlenderの連番名で新規作成します。
...
Curve表示上の扁平断面は廃止済みです。扁平化はCARDプレビューまたは扁平メッシュ出力を使用してください。

## カーブ形状設定

**カーブ形状設定** パネルでは、生成時と一括適用で使うCurveの見た目をまとめて設定します。

カーブ形状設定内のツイスト設定は折りたたみ式です。通常は非表示にしておき、ツイストを調整する時だけ展開してください。詳細/互換設定UIは廃止済みです。

### 基本

- 毛束長さ(cm): 新規生成Curveの基本長さです。通常はこの現在値がCurve生成に使われます。
- カーブの太さ(cm): Curve Bevel Depthとして使う太さです。内部ではmに変換します。
- 配置点の推奨長さを使用: ONにすると、配置点生成時に保存された長さを使用します。配置点生成後に毛束長さを変更したい場合はOFFにしてください。
- 解像度: Curveの滑らかさです。
- セグメント数: 新規生成時の制御点数です。
...
Hair Guide Designerでは、内部計算はBlender標準のm単位で行います。ただしUIでは、毛束長さ・Curve太さ・CARD幅・扁平メッシュ幅/厚み・根元/中間/毛先の位置ブレ・配置点の幅/高さ/奥行き揺らぎをcm単位で入力できます。
...
9. 配置点の揺らぎは初期値0のため左右対称。揺らぎ値やSeedを変えるとランダム変化が加わる。
