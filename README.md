...
8. **整理・削除** `[CLEANUP]` / `TRASH`: CARD Preview削除、Flat Mesh Preview削除、配置点削除、ガイド削除、Warning削除、未使用CARD Control Empty削除、`hair_guide`へ整理、全生成物削除、作業終了。検証系操作は通常UIに表示しません。
9. **詳細設定**: 互換Property、低頻度設定、デバッグ/保守用Operatorを格納します。通常UIには表示しません。
...
**基本ガイドを生成** は、上記8本の基本ガイドを作成します。既存の基本ガイドがある場合は、同じ固定名とその `.001` / `.002` などの派生名を先に完全削除してから作り直します。再生成後も `HAIR_GUIDE_SideBoundary_L/R` などを固定名で参照できます。配置点、カーブ、警告、頭部メッシュは削除しません。

基本ガイドの初期位置は頭部Bounding Boxを基準にします。`HAIR_GUIDE_Hairline` は額前面側、`HAIR_GUIDE_BackVolume`、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape` は後頭部・首側の外側へ余裕を持って生成します。Front/Backガイドは頭部に沿う浅い弧として生成されます。BackUpperは頭頂から襟足へ38%、BackMiddleは62%の高さを目安にした左右方向Curveです。多少の頭部メッシュへの食い込みは許容し、直線よりも髪の流れを把握しやすい形を優先しています。`HAIR_GUIDE_SideBoundary_L/R` はFrontとBackを端から端まで長く横断する線ではなく、Front HairlineとBack Volumeの間をつなぐ短めの補助ガイドとして生成します。
...
ガイド/領域表示は1ボタンのトグル式です。対象が1つでも表示中なら非表示へ、すべて非表示なら表示へ切り替わります。
...
**配置点を生成/更新** は、現在の基本ガイド位置を優先して参照します。ユーザーが `HAIR_GUIDE_Hairline`、`HAIR_GUIDE_SideBoundary_L/R`、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape`、`HAIR_GUIDE_Center` をObject Modeで移動・回転・拡縮した場合、そのWorld座標を使って配置点が再生成されます。`HAIR_GUIDE_BackVolume` は互換・表示用として残りますが、後頭部上層/中層の通常配置点生成には使用しません。
...
Back_Middleも他領域と同じ通常ループで配置点を生成します。`POINT_Back_Middle_01_L` / `POINT_Back_Middle_01_R` などの固定名は既存互換のため維持しますが、左右対称性やペア感は特殊なX反転処理ではなく `HAIR_GUIDE_BackMiddle` の形状で制御します。
...
Back_Upper / 後頭部上層の配置点は `HAIR_GUIDE_BackUpper`、Back_Middle / 後頭部中層の配置点は `HAIR_GUIDE_BackMiddle` から直接サンプリングして生成されます。配置点の位置を変えたい場合は、それぞれの専用ガイドCurveをObject Mode/Edit Modeで編集してください。補完式ではなく、Front / Side / Nape と同じくガイド上サンプリングで生成され、専用ガイドから取得した位置へ追加の後方オフセットは重ねません。旧ファイルで後頭部専用ガイドが無い場合のみ、警告を出したうえでBackVolume / Hachi / Occipitalなどを使う互換フォールバックで生成します。
...
**Curve原点を参照Emptyへ移動** がONの場合、Curve生成時に参照Emptyが割り当てられていれば、Curveの見た目とWorld座標を維持したままObject Originを参照Empty位置へ自動移動します。ツイスト生成では表示用 `twist_strand` ではなく編集対象の `twist_control` の原点を先に整えてから表示用Curveを再生成します。既存Curveは **選択Curve原点を参照Emptyへ移動** で同じ処理を手動適用できます。CARD Preview / Flat Mesh Previewが古い場合はPreview更新を押してください。
...
- 前後ガイドを左右対称化 (`hgd.symmetrize_front_back_guides`): `HAIR_GUIDE_Hairline`、`HAIR_GUIDE_BackVolume`、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape` の1本のガイド内の左右差を整えます。手動調整後に使ってください。
- 左側→右側へミラー
- 右側→左側へミラー
...
## Mirror Modifierミラー / 非破壊左右確認

Mirror Modifierミラーを使うと、作業中は片側のCurveだけを編集しながら、反対側をMirror Modifierで非破壊表示できます。対象は生成Curve、`twist_control`、表示用`twist_strand`、CARD Preview、Flat Mesh Previewです。確定出力物であるCARD Mesh / Flat Mesh、配置点、ガイド、リージョン、警告は対象外です。

Display ModeパネルのMirror欄で以下を操作できます。

- **Mirror Modifierミラー**: Mirror Modifier方式の作業フローを使うためのON/OFFです。
- **ミラー軸**: 登録頭部の中心X、World X=0、または指定したMirror Emptyを基準にします。頭部中心Xの場合も内部的にはMirror Emptyを作成してMirror Objectとして使います。
- **Mirror Empty作成**: `hair_guide / 05_Empties` にMirror用Emptyを作成し、Sceneへ登録します。
- **選択対象へMirror適用**: 選択中のCurve / Previewへ `HGD_Mirror` Modifierを追加します。
- **全Curve/PreviewへMirror適用**: 生成済みの対象Curve / Preview全体へ `HGD_Mirror` Modifierを追加します。
- **Mirror解決**: Mirror Modifierで表示されている反対側を、独立したCurveとPreviewとして生成します。元Curveは残り、反対側Curveも個別編集できます。出力Meshは解決対象外です。
- **Mirror Modifier削除**: 選択対象、または未選択時は全対象から `HGD_Mirror` を削除します。

推奨フローは、片側CurveとPreviewでMirror表示を確認し、Mirror解決で左右独立Curve/Previewにした後、CARD MeshまたはFlat Meshを出力する順序です。Mirror解決時に「解決後Mirror Modifierを削除」がONの場合、元Curve/Previewから `HGD_Mirror` を削除します。OFFの場合は元側の非破壊Mirror表示を維持しますが、解決された反対側ObjectにはMirror Modifierを付けません。
...
「作業終了：出力Meshのみ残す」を実行すると、確認ダイアログの後、ガイド・配置点・Curve・Preview・Empty・Warningを削除し、CARD Mesh / Flat Meshのみを残します。

この操作は作業用ガイドを削除するため、必要ならBlendファイルを保存してから実行してください。
...
扁平メッシュの確定出力は「出力」Panelの扁平メッシュ設定から行います。Preview (`flat_mesh_preview`) と確定Mesh (`flat_mesh`) は同じ生成ロジックを使うため、CARD Previewと左右端位置が一致します。Flat MeshはCARD設定に準拠し、Root/Mid/Tip幅、Mid位置、幅補間、CARDサンプル数、CARD Control Empty方向を参照します。そのため、Flat Meshの太さや長さ方向の分割を変えたい場合はCARD設定を変更してください。Flat Mesh側で個別に指定するものは、厚み、断面分割数、Sharp、Subdivisionのみです。断面分割数は4〜32で、各Curveサンプルごとに楕円リング断面を作成し、4では四角断面に近く、8/16/32ではより滑らかな扁平断面になります。
