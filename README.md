...
3. **ガイド・配置点** `[GUIDE]` / `OUTLINER_OB_CURVE`: ガイド表示、基本ガイド生成、配置点生成/削除、領域表示、前後ガイド左右対称化。
...
基本ガイドの初期位置は頭部Bounding Boxを基準にします。`HAIR_GUIDE_Hairline` は額前面側、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape` は後頭部・首側の外側へ余裕を持って生成します。Front/Backガイドは頭部に沿う浅い弧として生成されます。BackUpperは頭頂から襟足へ38%、BackMiddleは62%の高さを目安にした左右方向Curveです。多少の頭部メッシュへの食い込みは許容し、直線よりも髪の流れを把握しやすい形を優先しています。`HAIR_GUIDE_SideBoundary_L/R` はFrontとBackを端から端まで長く横断する線ではなく、Front Hairlineと後頭部専用ガイドの間をつなぐ短めの補助ガイドとして生成します。
...
**配置点を生成/更新** は、現在の基本ガイド位置を優先して参照します。ユーザーが `HAIR_GUIDE_Hairline`、`HAIR_GUIDE_SideBoundary_L/R`、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape`、`HAIR_GUIDE_Center` をObject Modeで移動・回転・拡縮した場合、そのWorld座標を使って配置点が再生成されます。後頭部配置点は `HAIR_GUIDE_BackUpper` / `HAIR_GUIDE_BackMiddle` / `HAIR_GUIDE_Nape` の専用ガイドから生成されます。`HAIR_GUIDE_BackVolume` は使用せず、新規生成もしません。
...
Back_Upper / 後頭部上層の配置点は `HAIR_GUIDE_BackUpper`、Back_Middle / 後頭部中層の配置点は `HAIR_GUIDE_BackMiddle` から直接サンプリングして生成されます。配置点の位置を変えたい場合は、それぞれの専用ガイドCurveをObject Mode/Edit Modeで編集してください。補完式ではなく、Front / Side / Nape と同じくガイド上サンプリングで生成され、専用ガイドから取得した位置へ追加の後方オフセットは重ねません。旧ファイルに `HAIR_GUIDE_BackVolume` が残っていても新規参照せず、後頭部専用ガイドが無い場合は配置点を生成しません。
...
Guide / Points Panelには、1本の前後系ガイド内の左右差を整える **前後ガイドを左右対称化** だけを残しています。対象は `HAIR_GUIDE_Hairline`、`HAIR_GUIDE_BackUpper`、`HAIR_GUIDE_BackMiddle`、`HAIR_GUIDE_Nape` です。

側頭部ガイド、配置点、生成Curve、ツイスト制御Curveを左右へミラーするHair Guide Designer内の管理機能は削除されました。左右対称が必要な場合は、通常のBlender操作または出力後のMesh編集で対応します。

## 左右対称編集について

Hair Guide Designer内のMirror Mode、Mirror Modifier管理、左右ミラー同期、Mirror解決機能は削除されました。左右対称が必要な場合は、通常のBlender操作または出力後のMesh編集で対応します。
