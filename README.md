...
Top / 頭頂部は、髪全体の流れの起点です。前髪・側頭部・後頭部へ毛流れを分けるために使います。Top / 頭頂部とBack_Upper / 後頭部上層の配置点は、頭部最高点より少し上（0.01m）に生成され、頭頂面へ埋まりにくくなっています。
...
表示モードの切り替えと適用ボタンは **クイック操作** パネルへ集約されています。CARD表示パネルにはCARD幅、CARD Mid位置、CARD方向制御、CARD Preview更新だけを残しています。太さ・Taper・位置ブレ・長さブレなどの数値は **カーブ編集** に集約されています。
...
CARD Mid位置やCARD幅補間を変更した後、既存CARDプレビューの見た目は自動更新されません。既存仕様通り、**CARDプレビューを更新** または **選択対象へ表示モード適用** を押して再生成してください。CARD幅プリセットはRoot/Mid/Tip幅だけを変更し、CARD Mid位置は変更しません。
...
Pie Menuには表示モード適用、CARD Preview更新、Flat Mesh Preview更新、Mesh出力、最終編集モード、参照Empty作成/共有、編集Curveを開く、Cleanup / Preview削除を配置しています。
