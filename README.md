...
環境やCurve状態によって扁平断面が表示されない場合があるため、暫定的に丸断面へFallbackする処理を入れています。安定した板状髪が必要な場合は、最終段階で手動メッシュ化または別途板ポリ化してください。
...
表示用の `HGD_BRAID_STRAND_A/B/C` は、単純ならせん状に回転する3本のツイストではありません。制御カーブに沿って左・中央・右のレーンを周期的に入れ替え、中央を通過する房に前後方向の差を付けることで、三つ編みに近い見た目を作ります。
...
- 編み込み数: 左・中央・右のレーン入れ替わり回数。多いほど細かく編み込まれます。
- 横幅: 三つ編み全体の左右幅。
- 交差の奥行き: 中央交差時の前後方向のふくらみ。大きいほど房の上下差が強く見えます。
- 毛先細り: 毛先へ向かう細り具合。
- 編み込み倍率: 編み込み周期の倍率。高いほど左右レーンの入れ替わりが増えます。
- 解像度: 制御カーブのサンプリング密度。
...
ガイドや配置点が頭部メッシュに隠れる場合は、**表示と削除** パネルの **最前面表示にする** を押してください。解除したい場合は同じ場所に表示される **最前面表示を解除** を押します。この設定は新規生成されるガイド、領域線、配置点、警告にも反映されます。
